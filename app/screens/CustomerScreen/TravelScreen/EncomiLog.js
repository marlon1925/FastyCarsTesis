...
export const EncomiLog = ({ navigation, route }) => {
    ...
    const { handleIsLoading, reservationLog, handleReservationLog, handleBoletoUpdate, resetReservationPriv, handleService, selectService } = useContext(UsuarioContext)
    useFocusEffect(
        useCallback(() => {
            getReservation();
        }, [])
    ...
    const getReservation = async () => {
        ...
        try {
            ...
            console.log("Repusta", bodyResponse.reservasClienteE)
            console.log("SERVICIO", service)
            service == "Encomienda" ? handleReservationLog(bodyResponse.reservasClienteE) : null
    ...
    const ContentPediente = () => {
        if (!reservationLog || reservationLog.length === 0) {
            ...
            return <Text>No hay encomiendas disponibles</Text>;
        } else {
            // Verificar si al menos un elemento cumple con la condición
            const tieneElementosValidos = reservationLog.some(element => element.estadoPaquete === "Pendiente");
            let fecha = reservationLog.map(element => element.turno.fecha === saveDate)
            ...
            return (
                <>
                    {reservationLog.map((element, index) => {
                        ...
                        if (saveDate ? element.estadoPaquete == "Pendiente" && element.turno.fecha === saveDate : element.estadoPaquete == "Pendiente") {
                            return (
                                    ...
                                    <View style={{ height: 180, borderWidth: 1.5, borderRadius: 18, padding: 15, justifyContent: "center", marginBottom: 10 }}>
                                        <View style={{ flexDirection: "row", flex: 1 }}>
                                            <View style={{ flex: 0.8, justifyContent: "center" }}>
                                                <Text style={{ fontSize: 20, fontFamily: theme.fonts.inter }}>{element.ciudadRemitente.ciudad + " - " + element.ciudadDestinatario.ciudad}</Text>
    ...
    const ContentFinalizado = () => {
        if (!reservationLog || reservationLog.length === 0) {
            ...
            // Verificar si al menos un elemento cumple con la condición
            const tieneElementosValidos = reservationLog.some(element => element.estadoPaquete === "Completado");
            ...
            return (
                <>
                    {reservationLog.map((element, index) => {
                        // Verificar la condición antes de renderizar el elemento
                        if (saveDate ? element.estadoPaquete == "Completado" && element.turno.fecha === saveDate : element.estadoPaquete == "Completado") {
                            return (
                                <TouchableOpacity key={index} onPress={() => {
                                    handleBoletoUpdate(element)
                                    navigation.navigate(ROUTES.RESUMEN_LOG)
                                    ...
                                    <View key={index} style={{ height: 180, borderWidth: 1.5, borderRadius: 18, padding: 15, justifyContent: "center", marginBottom: 10 }}>
                                        <View style={{ flexDirection: "row", flex: 1 }}>
                                            <View style={{ flex: 0.8, justifyContent: "center" }}>
                                                <Text style={{ fontSize: 20, fontFamily: theme.fonts.inter }}>{element.ciudadRemitente.ciudad + " - " + element.ciudadDestinatario.ciudad}</Text>
                                                <Text style={{ fontSize: 15, fontFamily: theme.fonts.inter }}>{element.turno.fecha}</Text>
    ...
    const ContentAprobados = () => {
        if (!reservationLog || reservationLog.length === 0) {
            ...
            // Verificar si al menos un elemento cumple con la condición
            const tieneElementosValidos = reservationLog.some(element => element.estadoPaquete === "Aprobado");
            ...
            return (
                <>
                    {reservationLog.map((element, index) => {
                        // Verificar la condición antes de renderizar el elemento
                        if (saveDate ? element.estadoPaquete == "Aprobado" && element.turno.fecha === saveDate : element.estadoPaquete == "Completado") {
                            return (
                                <TouchableOpacity key={index} onPress={() => {
                                    handleBoletoUpdate(element)
                                    navigation.navigate(ROUTES.RESUMEN_LOG)
    ...
    const ContentAccept = () => {
        if (!reservationLog || reservationLog.length === 0) {
            ...
            // Verificar si al menos un elemento cumple con la condición
            const tieneElementosValidos = reservationLog.some(element => element.estadoPaquete === "En tránsito" || element.estadoPaquete === "Aprobado");
            ...
            return (
                <>
                    {reservationLog.map((element, index) => {
                        // Verificar la condición antes de renderizar el elemento
                        if (saveDate ? element.estadoPaquete == "En tránsito" && element.turno.fecha === saveDate : element.estadoPaquete == "En tránsito") {
                            return (
                                <TouchableOpacity key={index} onPress={() => {
                                    handleBoletoUpdate(element)
                                    navigation.navigate(ROUTES.RESUMEN_LOG)
        ...
        return (
            <View style={{ flexDirection: "row", justifyContent: "center", alignItems: "center" }}>
                <Button style={styles.buttonContent} onPress={() => {
                    service == "Compartido" ? navigation.navigate(ROUTES.RESERVATION_EDIT) : navigation.navigate(ROUTES.HOME_ENCOMIENDA_EDIT)
    ...
    return (
        ...
        >
            ...
            <View style={styles.contentContainer}>
                ...
                <View style={{ justifyContent: "flex-start", marginBottom: 25, marginTop: 15 }}>
                    <Text style={{ fontWeight: theme.fonts.interBold, fontSize: theme.fontSize.titleHeader, textAlign: "center", fontWeight: "700" }}> Estados de las encomiendas</Text>
                </View>
                <View style={{ height: "10%", flexDirection: "row" }}>
                    ...
                    <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
                        <ButtonStyledImg fontSize={12} text={" Aprobados "} onPress={() => setSelect("aprobado")} color={select == 'aprobado' ? theme.colors.Blue : 'white'} width={"90%"} />
                    </View>
                    <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
                ...
                >
                    {
                        select == 'pendiente' ? <ContentPediente /> : select == 'finalizado' ? <ContentFinalizado /> : select == "aprobado" ? <ContentAprobados /> : <ContentAccept />
                    }
                </ScrollView>
            </View>
