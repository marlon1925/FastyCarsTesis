...
export const EditPerfilDriver = ({ navigation }) => {
    const { userInfo, handleIsLoading, handleUserInfo } = useContext(UsuarioContext)
    const [registerForm, setRegisterForm] = useState({
        conductorNombre: userInfo.msg.conductorNombre,
        conductorApellido: userInfo.msg.conductorApellido,
        phone: "0" + userInfo.msg.phone,
    ...
    const updatePerfil = async () => {
        ...
        try {
            ...
            const url = `${baseURL()}chofer/actualizarPerfil/${userInfo.msg._id}`;
            const options = {
                method: 'PUT',
            ...
            console.log("RESPUESTA: ", bodyResponse)
            if (respuesta.ok) {
                        ...
                        <View style={{ flex: 1, justifyContent: "flex-start" }}>
                            <View style={{ marginBottom: 30 }}>
                                <TextInput
                                    ...
                                    label="Nombre"
                                    value={registerForm.conductorNombre}
                                    keyboardType="email-address"
                                    onChangeText={(txt) => {
                                        setRegisterForm({ ...registerForm, conductorNombre: txt.trim() });
                                        let validate = valitdatename(txt.trim())
                                        setErrors({ ...errors, conductorNombre: validate })
                                    }}
                                    mode="outlined"
                                    placeholder="Nombre"
                                    outlineColor="#666666"
                                    ...
                                    maxLength={10}
                                    error={errors.conductorNombre}
                                />
                                {errors.conductorNombre && (
                                    <HelperText
                                        type="error"
                                        visible={errors.conductorNombre}
                            ...
                            <View style={{ marginBottom: 30 }}>
                                <TextInput
                                    autoCapitFalize="none"
                                    label="Apellido"
                                    value={registerForm.conductorApellido}
                                    keyboardType="email-address"
                                    onChangeText={(txt) => {
                                        setRegisterForm({ ...registerForm, conductorApellido: txt.trim() });
                                        let validate = valitdatename(txt.trim())
                                        setErrors({ ...errors, conductorApellido: validate })
                                    }}
                                    mode="outlined"
                                    ...
                                    style={styles.inputStyle}
                                    maxLength={10}
                                    error={errors.conductorApellido}
                                />
                                {errors.conductorApellido && (
                                    <HelperText type="error" visible={errors.conductorApellido}> {errors.conductorApellido} </HelperText>
                                )}
                            </View>
                            {/* <View style={{ flex: 1 }}>
                                <TextInput
